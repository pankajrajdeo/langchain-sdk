...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| [`langgraph dev`](https://docs.langchain.com/langsmith/cli#dev)               | Starts a lightweight local dev server (no Docker required), ideal for rapid testing.                                                 |
| [`langgraph build`](https://docs.langchain.com/langsmith/cli#build)           | Builds a Docker image of your LangGraph API server for deployment.                                                                   |
| [`langgraph deploy`](https://docs.langchain.com/langsmith/cli#deploy)         | Builds and deploys a LangGraph image directly to LangSmith Deployments in a single step.                                             |
| [`langgraph dockerfile`](https://docs.langchain.com/langsmith/cli#dockerfile) | Emits a Dockerfile derived from your config for custom builds.                                                                       |
| [`langgraph up`](https://docs.langchain.com/langsmith/cli#up)                 | Starts the LangGraph API server locally in Docker. Requires Docker running; LangSmith API key for local dev; license for production. |
...
| <span>`graphs`</span>           | **Required**. Mapping from graph ID to path where the compiled graph or a function that makes a graph is defined. Example: <ul><li>`./your_package/your_file.py:variable`, where `variable` is an instance of `langgraph.graph.state.CompiledStateGraph`</li><li>`./your_package/your_file.py:make_graph`, where `make_graph` is a function that takes a config dictionary (`langchain_core.runnables.RunnableConfig`) and returns an instance of `langgraph.graph.state.StateGraph` or `langgraph.graph.state.CompiledStateGraph`. See [how to rebuild a graph at runtime](https://docs.langchain.com/langsmith/graph-rebuild) for more details.</li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| <span>`auth`</span>             | *(Added in v0.0.11)* Auth configuration containing the path to your authentication handler. Example: `./your_package/auth.py:auth`, where `auth` is an instance of `langgraph_sdk.Auth`. See [authentication guide](https://docs.langchain.com/langsmith/auth) for details.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
All deployments come with a DB-backed BaseStore. Adding an "index" configuration to your `langgraph.json` will enable [semantic search](https://docs.langchain.com/langsmith/semantic-search) within the BaseStore of your deployment.
...
See the [authentication conceptual guide](https://docs.langchain.com/langsmith/auth) for details, and the [setting up custom authentication](https://docs.langchain.com/langsmith/set-up-custom-auth) guide for a practical walk through of the process.
...
> [!NOTE]
>   Customizing your server's CORS configuration will override the functionality of setting the [`CORS_ALLOW_ORIGINS` environment variable](https://docs.langchain.com/langsmith/env-var-cloud).
...
See [Use webhooks](https://docs.langchain.com/langsmith/use-webhooks#add-headers-to-webhook-requests) for details on header configuration, environment variable templating, and URL restrictions.
...
Other route disable flags include `disable_assistants`, `disable_runs`, `disable_threads`, `disable_store`, and `disable_ui`. For MCP, A2A, and webhooks, see their respective guides: [Disable MCP](https://docs.langchain.com/langsmith/server-mcp#disable-mcp), [Disable A2A](https://docs.langchain.com/langsmith/server-a2a#disable-a2a), [Disable webhooks](https://docs.langchain.com/langsmith/use-webhooks#disable-webhooks).
...
> [!TIP]
> If you need more information on when to use `langgraph dev` vs `langgraph up`, refer to the [Local development & testing guide](https://docs.langchain.com/langsmith/local-dev-testing) for a detailed comparison.
...
#### Python
This command is in [beta](https://docs.langchain.com/langsmith/release-stages) and under active development. Expect frequent updates and improvements.

Build and deploy a LangGraph image directly to [LangSmith Deployments](https://docs.langchain.com/langsmith/deployment). This command builds a Docker image locally, pushes it to a managed registry, and creates or updates a deployment—all in a single step. If Docker is not installed, it triggers a remote build.

**Prerequisites**

* A [**LangSmith API key**](https://docs.langchain.com/langsmith/create-account-api-key) with access to Deployments.
...
This command also accepts all [`langgraph build`](https://docs.langchain.com/langsmith/cli#build) flags (`--platform`, `-t`, `--pull`, `--no-pull`, `-c`). For details, refer to `langgraph build --help`.
...
Use [`deploy list`](https://docs.langchain.com/langsmith/cli#deploy-list) to list deployment IDs.
...
Use [`deploy list`](https://docs.langchain.com/langsmith/cli#deploy-list) to find the deployment ID to delete.
...
| Option              | Default          | Description                                                                                                     |
| ------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------- |
| `-c, --config FILE` | `langgraph.json` | Path to the [configuration file](https://docs.langchain.com/langsmith/cli#configuration-file) declaring dependencies, graphs and environment variables. |
| `--help`            |                  | Show this message and exit.                                                                                     |
