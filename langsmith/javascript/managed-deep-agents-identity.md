...
> Source: [Original LangChain documentation](https://docs.langchain.com/langsmith/javascript/managed-deep-agents-identity)
...
Anyone with the key can reach the deployment, so treat the key as a secret. This default does not give each end user private threads. If Alice must not see Bob's threads, use [Supabase](https://docs.langchain.com/langsmith/javascript/managed-deep-agents-identity#authenticate-end-users-with-supabase).
