...
* **`agent.py`**: Defines and exports the agent. See [Agent definition](managed-deep-agents-agent-definition.md).
...
This example uses an [OpenAI chat model](../../integrations/chat/openai.md). If you choose another model provider, add the API key required by that provider instead. `mda deploy` uses the LangSmith API key to deploy the agent and adds the provider key to the deployment.
...
The model handles the agent's language understanding and reasoning. The agent name is also the default deployment name. For model concepts and provider options, see [Models](../../langchain/models.md).
...
A tool is a function the agent can call to retrieve data or take an action. Choose your model provider's server-side search or create a [custom LangChain tool](../../langchain/tools.md) with Tavily.
