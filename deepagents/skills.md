...
This pattern is useful when different users or tenants need fully independent skill libraries that can be updated separately. For a managed solution that handles skill access, sharing, and workspace-level visibility out of the box, see [Fleet skills](../langsmith/fleet/skills.md).
...
Production deployments usually need to control three things: which skills each user can see, whether the agent can modify skill files, and whether writes require human approval. You control visibility with the `skills` argument and [backend routing](#backends-and-remote-skill-loading), access with [filesystem permissions](permissions.md), and approval with [`interrupt_on`](human-in-the-loop.md) or permission rules with `mode="interrupt"`.
...
For a managed solution that handles skill access, sharing, and workspace-level visibility, see [Fleet skills](../langsmith/fleet/skills.md).
...
3. Do not add a `deny` rule for the writable path. Place more specific rules before broader deny rules if you mix shared and personal paths ([rule ordering](permissions.md#rule-ordering)).
...
The agent uses `write_file` and `edit_file` to create or update `SKILL.md` and supporting files under the writable path. To capture general learnings outside the skills format, route a separate path such as `/memories/` to another writable backend. See [Backends](backends.md) for routing and store setup.
