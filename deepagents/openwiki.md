...
OpenWiki is an open source CLI that writes and maintains a Markdown wiki about your codebase or personal knowledge. The wiki captures details such as architecture, integrations, evals, and workflows so [coding agents](overview.md) can use it as durable context instead of rediscovering the repository on every task.

That makes agent work faster and cheaper in tokens: agents read a curated wiki first, then inspect source only where they need more detail. Humans can browse the same Markdown (and the local [visualizer](../openwiki/visualize.md)), but the primary audience is agents.

OpenWiki is built on [Deep Agents](overview.md) and supports tracing with [LangSmith](../langsmith/observability-quickstart.md).
...
See the [Quickstart](../openwiki/quickstart.md) to choose a model provider, generate docs, and keep them up to date.
...
#### [CLI reference](../openwiki/cli-reference.md)
